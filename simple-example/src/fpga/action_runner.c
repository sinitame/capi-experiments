 ...
 * A vector of size N is generated by the action
 * 
 */
...
static void snap_prepare_action(struct snap_job *cjob,
		...
		uint8_t type_out)
{
	fprintf(stderr, "  prepare action job of %ld bytes size\n", sizeof(*mjob));
...
/* ACTION RUNNER for the vector generator action example                 */
...
int main(int argc, char *argv[])
{
	...
	snap_prepare_action(&cjob, &mjob,vectorSize,(void *)addr_out, vectorSize*sizeof(uint32_t), type_out);
	...
	fprintf(stdout, "Vector generated : [%d,%d,%d, ... ,%d]\n",obuff[0],obuff[1],obuff[2],obuff[vectorSize-1]);

	/* If the output buffer is in host DRAM we can write it to a file */
	if (output != NULL) {
		fprintf(stdout, "Writing output data from @%p of %ld bytes to %s\n", obuff, (int)vectorSize*sizeof(uint32_t), output);
